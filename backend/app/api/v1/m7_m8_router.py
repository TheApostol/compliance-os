...
async def simulate_market_entry(
    req: ExpansionSimulationRequest,
    response: Response,
    current_user: CurrentUser = Depends(get_current_user),
):
    result = await get_predictive_engine().simulate_market_entry(
        business_model=req.business_model,
        countries=req.countries,
        tenant_id=current_user.tenant_id,
    )
    source = result.get("_source", "ai")
    response.headers["X-Score-Source"] = source
    return result
