"""
SFC Crawler — Superintendencia Financiera de Colombia
======================================================
Fetches Circulares and Resoluciones from SFC's publication index.
Index URL: https://www.superfinanciera.gov.co/jsp/loader.jsf?lServicio=Publicaciones&lTipo=publicaciones&lFuncion=loadContenidoPublicacion&id=61491
PDFs served at: https://www.superfinanciera.gov.co
"""
...
SFC_INDEX_URL = (
    ...
    "?lServicio=Publicaciones&lTipo=publicaciones"
    "&lFuncion=loadContenidoPublicacion&id=61491"
)
SFC_PDF_BASE = "https://www.superfinanciera.gov.co"

# Known-good fallback: 5 SFC circulars
# Used when the index page is unavailable or returns a non-200 status.
SFC_FALLBACK_URLS: list[str] = [
    f"https://www.superfinanciera.gov.co/descargas?com=institucional&name=pubFile{num}&downloadname={num}.pdf"
    for num in [10097650, 10097651, 10097652, 10097653, 10097654]
]


class SFCCrawler(BaseCrawler):
    def __init__(self):
        super().__init__(regulator="SFC", country="CO")

    async def fetch_index(self) -> list[IndexEntry]:
        """Scrape SFC normatives index and return up to 50 recent entries.

        Falls back to a hardcoded list of recent known-good URLs if the index
        page is unavailable or returns a non-200 status.
        """
        try:
            resp = await self._get(SFC_INDEX_URL)
        except Exception as e:
            logger.warning("SFC index network error — using fallback URLs: %s", e)
            return self._fallback_entries()

        if resp.status_code != 200:
            logger.warning(
                "SFC index returned HTTP %s — using fallback URLs", resp.status_code
            )
            return self._fallback_entries()

        entries = self._parse_index_html(resp.text)
        if not entries:
            logger.warning("SFC index parsed 0 entries — using fallback URLs")
            return self._fallback_entries()
    ...
    def _fallback_entries() -> list[IndexEntry]:
        ...
        return [
            IndexEntry(
                url=url,
                title=f"SFC {url.split('pubFile')[-1].split('&')[0]} (fallback)",
            )
            for url in SFC_FALLBACK_URLS
        ]
    ...
    def _parse_index_html(self, html: str) -> list[IndexEntry]:
        ...
        if len(entries) >= 3:
            return entries[:50]
        # Strategy 2: table row parsing — rows containing Circular/Resolución/Instrucción
        entries = self._strategy_table_rows(html)
        if len(entries) >= 3:
            return entries[:50]
        # Strategy 3: anchors matching Circular/CE pattern
    ...
    def _strategy_pdf_links(self, html: str) -> list[IndexEntry]:
        ...
        for match in pattern.finditer(html):
            ...
            url = href if href.startswith("http") else f"{SFC_PDF_BASE}{href}"
    ...
    def _strategy_table_rows(self, html: str) -> list[IndexEntry]:
        """Strategy 2: find table rows containing Circular/Resolución/Instrucción keywords."""
        entries: list[IndexEntry] = []
        rows = re.findall(
            r"<tr[^>]*>.*?</tr>",
            html,
        ...
        keyword_pattern = re.compile(
            r"Circular|Resoluci[oó]n|Instrucci[oó]n",
            re.IGNORECASE,
        )
        for row in rows:
    ...
    def _strategy_anchor_text(self, html: str) -> list[IndexEntry]:
        """Strategy 3: find anchors whose text matches Circular/CE pattern."""
        entries: list[IndexEntry] = []
        norm_pattern = re.compile(
            r"Circular\s*(?:B[aá]sica|Externa|Jur[íi]dica)?\s*\d*|CE\s*\d+|"
            r"Circular|Resoluci[oó]n|Resolucio|Instrucci[oó]n|Instruccion",
            re.IGNORECASE,
        )
